...
if __name__ == "__main__":
    ...
    pyramid = rcnn.image_pyramid(orig, scale=rcnn.PYR_SCALE, minSize=rcnn.ROI_SIZE)
    ...
    for image in pyramid:
        ...
        # window locations
        for (x, y, roiOrig) in rcnn.sliding_window(image, rcnn.WIN_STEP, rcnn.ROI_SIZE):
            # scale the (x, y)-coordinates of the ROI with respect to the
            # *original* image dimensions
            x = int(x * scale)
            y = int(y * scale)
            w = int(rcnn.ROI_SIZE[0] * scale)
            h = int(rcnn.ROI_SIZE[1] * scale)
            # take the ROI and preprocess it so we can later classify
            # the region using Keras/TensorFlow
            roi = cv2.resize(roiOrig, rcnn.INPUT_SIZE)
